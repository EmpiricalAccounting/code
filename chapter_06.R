...
financial_data |>
  ...
  labs(title = "Histgram of ROA", x = "ROA", y = "Count") +
  theme_classic()
...
bin_quantiles <- financial_data |>
  group_by(bin) |>
  summarize(
      q_25  = quantile(wac, 0.25, na.rm = TRUE),
  ...
  # 区間の中央値
  mutate(
    bin_mid = (breaks[as.integer(bin)] + breaks[as.integer(bin) + 1]) / 2
  )
