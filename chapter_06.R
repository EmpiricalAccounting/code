...
quantiles_df <- benchmark  |>
  group_by(bin) |>
  summarize(
    q0 = quantile(WAC, 0, na.rm = TRUE),
    q25 = quantile(WAC, 0.25, na.rm = TRUE),
    q50 = quantile(WAC, 0.5, na.rm = TRUE),
    q75 = quantile(WAC, 0.75, na.rm = TRUE),
    q100 = quantile(WAC, 1, na.rm = TRUE)
  )  |>
  mutate(
