...
cleaned_df <- merged_df |>
  ...
  select(number, year, name, code, industory, market, standard, div, so, sale, ni, na, ta, roe, sp)

# 結果の表示
head(cleaned_df)

# EPS（1株あたり当期純利益）を計算して列を追加
cleaned_df$eps <- cleaned_df$ni / cleaned_df$so

# BPS（1株あたり純資産）を計算して列を追加
cleaned_df$bps <- cleaned_df$na / cleaned_df$so




# 必要ならCSVに保存
write.csv(cleaned_df, "05_eps_result.csv", row.names = FALSE)
